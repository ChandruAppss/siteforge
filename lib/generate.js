...
// Output length is what drives latency — models stream at a roughly fixed
...
/* Resolved per call rather than at import time, so a serverless instance does
   not cache a stale provider across an env change. */
export function currentModel() {
  return resolveProvider().model || 'unconfigured';
}

/** Back-compat: several call sites still read MODEL as a plain value. */
...
export async function streamWebsite({ businessName, businessDescription, styleIndex, onChunk }) {
  ...
  if (process.env.MOCK_GENERATION === '1') {
    // MOCK_FAIL lets the failure path be exercised without burning credit.
    if (process.env.MOCK_FAIL) {
      await new Promise((r) => setTimeout(r, 300));
      throw new Error(process.env.MOCK_FAIL === '1'
  ...
  const raw = await provider.stream({
    prompt: buildPrompt(businessName, businessDescription, style),
    model,
  ...
  if (!/<html[\s>]/i.test(html) || html.length < 800) {
    throw new Error(`${provider.label} did not return a complete HTML document.`);
  }
 ...
 * @returns {Promise<{style:string, styleNote:string, html:string}>}
 */
export async function generateWebsite(args) {
  // Every provider adapter is streaming-only, so the non-streaming path is
  // just the streaming one with the chunks discarded.
  return streamWebsite({ ...args, onChunk: null });
}
...
function mockPage(businessName, style) {
  ...
  // A real generation emits several KB of <head> and <style> before any visible
  // body content. The mock must do the same or it cannot exercise the loading
  // UI, which exists precisely to cover that blank window.
  const filler = Array.from({ length: 40 }, (_, i) =>
    `.mock-utility-${i}{margin:${i}px;padding:${i}px;line-height:1.${i};letter-spacing:0.0${i}em}`
  ).join('\n');
